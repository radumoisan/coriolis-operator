...
2. Perform collision-safe preflight for all five frozen resources before mutation: classify metadata before retained Secret semantic validation, map semantic failures fail-closed to `COLLISION`, and invoke credential generators only for `ABSENT`. Then add SSA, minimal Secret RBAC, and reconciliation/status failure semantics. The pure renderer is committed locally at `97153a7`; decoded values remain internal and must never be logged, statused, or evented. TLS/optional credentials/storage/probes/readiness/bootstrap/rotation remain deferred. No runtime workloads are implemented or deployed.
