...
- CSI and cross-node attach/detach evidence remain open beyond the accepted single-node POC. Production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Resolve the overlapping Argo-tracked legacy/current Deployment selectors before shared-namespace appliance testing.
...
3. MariaDB reconciliation includes stable configuration failure, ordered pre-reads, exact retained-PVC reuse, guarded managed-resource writes, marker-last ordering, and narrow PVC/StatefulSet RBAC. Released `0.5.6` closes the accepted single-node `local-path` POC for clean first boot, RWO, fsGroup, retained reuse, same-node remount, probes, persistence, and 30-second termination. CSI attach/detach/rescheduling remains a later environment gate. Production backup/restore, HA, and RPO/RTO acceptance remain separate gates. RabbitMQ and Memcached workload evidence is next; Barbican and all other backend Services remain deferred.
