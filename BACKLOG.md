...
- MariaDB remains blocked on the Kubernetes workload, storage, manifest, probe, resource, lifecycle/recovery, and reliable log-capture contracts; the CRD has no storage configuration. No source/runtime code, CRD, workload, PVC, Job, RBAC, probe/readiness, release/version, deployment, or cluster state changed. Deployed `0.5.3` remains marker-only.
...
3. Close only the remaining MariaDB Kubernetes contract: workload kind, storage/PVC ownership and fsGroup behavior, access/retention, exact generated manifests, probes, resources, lifecycle/recovery, and reliable log capture. Implement builders and reconciliation only after eligibility. Barbican and all other backend Services remain deferred.
