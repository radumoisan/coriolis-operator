...
- Validation passed: 44 unit tests; `uv run ruff check .`; `uv run ruff format --check .`; `uv run mypy src`; `git diff --check` was clean before doc alignment. The helpers are uncommitted and unpushed; the deployed marker `0.5.3` is unchanged and carries no standard labels.
