...
3. Implement and test the pure MariaDB API validation/resolver and manifest-builder/preflight contract, then add reconciliation only after those tests pass. Target-storage validation, cluster deployment, and production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Barbican and all other backend Services remain deferred.
