...
- OCI/configuration/security/storage/bootstrap/probe/readiness evidence remains blocked for every dependency; MariaDB requires single-node, PVC, startup, and probe facts before any vertical slice. No workload, Job, PVC, RBAC, readiness, or runtime behavior changed.
...
3. Complete OCI, configuration, storage, bootstrap, and probe contracts, starting with MariaDB; implement pure builders and reconciliation only after eligibility. Barbican and all other backend Services remain deferred.
