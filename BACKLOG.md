...
## Completed Locally: MariaDB Reconciliation

- Local, unpushed, and undeployed: wired complete MariaDB settings into reconciliation with a stable, mutation-free `InvalidRuntimeConfiguration` status before Kubernetes client construction. Added storage/resources field handlers so corrected configuration and supported resource changes reconcile without broad child-resource watches.
...
2. The marker-plus-four foundational runtime gate is committed locally at `862777d`, with status commit `f219977`; both are unpushed and undeployed. The four-Service slice is committed locally at `797235b` on `dev`, unpushed and undeployed, and implements only RabbitMQ, Memcached, MariaDB, and Keystone; no runtime workloads are implemented or deployed.
3. MariaDB pure desired-state preparation and reconciliation are implemented and tested locally, including stable configuration failure, ordered pre-reads, exact retained-PVC reuse, guarded managed-resource writes, marker-last ordering, and narrow PVC/StatefulSet RBAC. Target-storage validation must close RWO, fsGroup, retained reuse, attach-detach/rescheduling, and clean 30-second termination before deployment. Production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Barbican and all other backend Services remain deferred.
