...
2. Define/package non-sensitive configuration rendering inputs/templates; perform collision-safe pre-reads for all frozen resources before mutation; combine metadata classification with retained Secret semantic validation (semantic failure fails closed as `COLLISION`); invoke generators only for `ABSENT` credentials; apply with SSA; add minimal Secret RBAC; and define reconciliation/status failure semantics. Helpers, classifiers, builders, generators, and semantic validation are committed locally through `5165629` but not pushed/deployed. Returned decoded values remain internal and must never be logged, statused, or evented. TLS/optional credentials/storage/probes/readiness/bootstrap/rotation remain deferred. No runtime workloads are implemented or deployed.
