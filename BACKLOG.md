...
2. Define/generate configuration rendering inputs, validate retained Secret semantic shape, then perform collision-safe pre-reads for all frozen resources before mutation, invoke generators only for `ABSENT` credentials, apply with SSA, add minimal Secret RBAC, and define reconciliation/status failure semantics. Helpers, classifiers, builders, and pure credential generators are committed locally through `a604579` but not pushed/deployed. TLS/optional credentials/storage/probes/readiness/bootstrap/rotation remain deferred. No runtime workloads are implemented or deployed.
