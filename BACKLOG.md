...
3. The pure MariaDB API validation/resolver and manifest-builder/preflight contract is implemented and tested locally. Next add reconciliation with stable configuration failure, ordered pre-reads, exact retained-PVC reuse, guarded managed-resource writes, marker-last ordering, narrow PVC/StatefulSet RBAC, and exhaustive tests. Target-storage validation, cluster deployment, and production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Barbican and all other backend Services remain deferred.
