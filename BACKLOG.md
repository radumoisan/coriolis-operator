...
- That historical contract changed no runtime behavior. The marker-plus-four foundational runtime gate is now complete locally; Services, Ingress, workloads, and remaining runtime design are deferred. Deployed `0.5.3` remains marker-only.
...
- Validation passed: `uv run pytest tests/unit` (243 passed); `uv run ruff check .`; `uv run ruff format --check .` (35 files already formatted); `uv run mypy src`; `helm lint helm/`; `helm template coriolis-operator helm/ --include-crds`; and `git diff --check`. The work is uncommitted, unpushed, and undeployed. No Services, Ingress resources, workloads, storage, bootstrap, probes/readiness, rotation, or deployment was added; deployed `0.5.3` remains unchanged and marker-only.
...
2. The marker-plus-four foundational runtime gate is complete locally and remains uncommitted, unpushed, and undeployed. Services, Ingress, workloads, and remaining runtime design are deferred; no runtime workloads are implemented or deployed.
