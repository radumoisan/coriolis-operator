...
## Active: MariaDB POC Follow-Up

- CIXpress pipeline `8nownk` succeeded for source `6ba9c7e`, and operator `0.5.5` was validated in a disposable namespace with the expected image digest. Clean first boot exposed anonymous accounts from MariaDB test-database initialization shadowing `coriolis@%`; the local `--skip-test-db` fix passes the full 316-test suite and all static/Helm checks, but a released fixed artifact still needs clean-first-boot validation.
- After controlled removal of only the disposable anonymous accounts, single-node `local-path` validation passed for RWO Filesystem provisioning, `fsGroup: 42434` writes, authenticated probes, exact retained-PVC/no-write credential reuse across CR recreation, persisted database state, and normal `1.684s` termination plus same-node remount. All disposable resources and the `Delete`-policy PV were removed.
- CSI and cross-node attach/detach evidence remain open beyond the accepted single-node POC. Production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Resolve the overlapping Argo-tracked `0.5.3` and `0.5.5` Deployment selectors before shared-namespace appliance testing.
...
2. The development stack through MariaDB reconciliation is published on `origin/dev` at `55212b0` and included in observed operator `0.5.5`. No runtime workloads remain deployed after POC cleanup.
3. MariaDB reconciliation includes stable configuration failure, ordered pre-reads, exact retained-PVC reuse, guarded managed-resource writes, marker-last ordering, and narrow PVC/StatefulSet RBAC. The accepted single-node `local-path` POC closed RWO, fsGroup, retained reuse, same-node remount, and 30-second termination behavior only after a controlled anonymous-account repair. Publish and clean-first-boot validate the local `--skip-test-db` fix next; CSI attach/detach/rescheduling remains a later environment gate. Production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Barbican and all other backend Services remain deferred.
