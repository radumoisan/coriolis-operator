...
## Completed Locally: Memcached Reconciliation

- Local, uncommitted, unpushed, and undeployed: implemented one owner-referenced ephemeral Memcached Deployment using the approved exact digest and direct `/usr/bin/memcached -p 11211 -U 0` startup. It has UID/GID `42457`, read-only root, dropped `ALL` capabilities, no privilege escalation, `RuntimeDefault`, no config, credentials, writable mounts, volumes, PVC, resources API, or init containers, plus protocol-level startup/readiness/liveness probes.
- Reconciliation reads the Deployment after MariaDB, completes all preflight before writes, creates absent or guarded-applies managed resources, leaves collisions mutation-free, writes after MariaDB before the marker, and adds only Deployment `get`/`create`/`patch` RBAC. `Ready=False/RuntimeNotImplemented` remains expected.
- Validation passed: `uv run --offline pytest tests/unit` (326 passed), Ruff lint/format, mypy, Helm lint/template, and `git diff --check`. No deployed Kubernetes Memcached POC or runtime resource, release, or chart version change is claimed. Next: review, publish, and run a released-artifact isolated POC; RabbitMQ evidence follows after Memcached acceptance.
...
- CSI and cross-node attach/detach evidence remain open beyond the accepted single-node POC. Production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Argo `0.5.7` has already removed the legacy Deployment selector overlap.
...
3. MariaDB reconciliation includes stable configuration failure, ordered pre-reads, exact retained-PVC reuse, guarded managed-resource writes, marker-last ordering, and narrow PVC/StatefulSet RBAC. Released `0.5.6` closes the accepted single-node `local-path` POC for clean first boot, RWO, fsGroup, retained reuse, same-node remount, probes, persistence, and 30-second termination. CSI attach/detach/rescheduling remains a later environment gate. Production backup/restore, HA, and RPO/RTO acceptance remain separate gates. Memcached is implemented locally but awaits review, publication, and a released-artifact isolated POC; RabbitMQ evidence follows after its acceptance. Barbican and all other backend Services remain deferred.
