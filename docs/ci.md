...
Chart version, application version, and image tag are synchronized at the provisional `0.0.0` baseline. The selected image repository is `cr.virtomat.io/virtomat/coriolis/operator`. CIXpress, not developers, owns future release-version edits.
...
CIXpress is experimental and non-transactional: artifacts published by earlier steps are not rolled back if a later step fails. CIXpress configuration and manifests are not yet in this repository. Publication/authentication validation for the selected repository, promotion, trigger and monitoring credentials, and deployment/GitOps remain separate integration work. Argo CRD pre-upgrade automation belongs to promotion/deployment and is not part of the standard build pipeline.
