...
Chart version, application version, and image tag are synchronized at release `0.5.3`. The selected image repository is `cr.virtomat.io/virtomat/coriolis/operator`. CIXpress, not developers, owns future release-version edits.
...
CIXpress is experimental and non-transactional: artifacts published by earlier steps are not rolled back if a later step fails. Full all-step pipeline evidence and CI image push/OCI chart publication validation are explicitly documented through release `0.5.2`; pipeline `5ly5kg` completed all expected steps as `SUCCEEDED`. Current chart/app/image synchronization and deployment are confirmed at `0.5.3` (Argo deployment `Synced`/`Healthy`, Deployment `1/1`), but no exact `0.5.3` pipeline ID or all-step evidence is recorded in the documentation; do not infer one. CIXpress configuration and manifests are not yet in this repository. Promotion policy, exact integration artifacts/templates/triggers/credentials, and Argo CRD pre-upgrade automation remain separate work.
