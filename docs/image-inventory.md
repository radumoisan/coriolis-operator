...
- The inventory and pull gates are **complete**. Memcached reconciliation is implemented in source `063e438ef416599e9816a2400afcc5a5a7af9aa0`, released as `0.5.8`, and passed an isolated single-node POC. RabbitMQ has local runtime evidence and an uncommitted, unpushed, undeployed implementation, but no released-artifact POC. The current Argo deployment is docs-only operator `0.5.9` with zero appliance CRs and no POC workload. Workloads/endpoints beyond released MariaDB and Memcached plus local RabbitMQ, Ingress, overall appliance readiness, and remaining runtime design are deferred.
