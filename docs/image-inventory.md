...
The tracked validator qualifies the exact `2603.4` scheduler image `sha256:45bea9e...a73c41` and transfer-cron image `sha256:3a44d3b...2b3ea9c` under their exact direct commands; both run as numeric UID/GID `42434` with read-only root, dropped `ALL`, no-new-privileges, read-only generated `/etc/coriolis`, writable only `/tmp` and `/var/log/coriolis`, on a private internal network with no host ports and no Service/listener. Scheduler source `d4805ea` is released as operator/chart `0.5.26`, and its isolated Kubernetes POC accepted the same exact scheduler digest, live restricted contract, replacement, RabbitMQ recovery, retained no-write recreation, and normal cleanup. Transfer-cron is now locally implemented and validated as an exact-image Deployment slice, but is not published or released and has no released-artifact Kubernetes POC. Its tracked validator passed `SUMMARY runtime passed 320.708`, including exact-image contract, fixed-output diagnostics after empty schedule load, stable runtime, graceful `0.792s` stop/restart, RabbitMQ recovery with unchanged workload identities, and cleanup with no containers, volumes, or networks remaining. These lifecycle results do not change the frozen digest table above.
