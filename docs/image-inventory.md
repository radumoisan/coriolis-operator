...
### :material-application-edit-outline: Coriolis Web Local Qualification

On 2026-08-26, exact image `cr.virtomat.io/virtomat/coriolis/coriolis-web:2603.4@sha256:32ebc391ac46fe627185694b3fd252afd7587b152f526dff38ae0a5b887c0db1` was qualified locally only. It is linux/amd64, root/unset user, workdir `/root/coriolis-web`, entrypoint `npm run start`, no Cmd, exposes `3000`, and declares no volume or healthcheck. `scripts/validate-coriolis-web-runtime.py` passed all 17 stages in `31.889s`: `GET /` and `/api/config` returned `200` with exact relative logical-origin URLs; the missing CA fingerprint endpoint returned expected `500` without destabilization; first-launch POST returned `200` then `false`; 20-second stability and recreation reset passed; and upstream stop exited exactly `1` in `0.747s` (bound `5s`). The run was fixed-output/body-suppressed with no network, host ports, mounts, devices, or leftovers. Non-root `42434` exited `243` before ready, read-only root made first-launch POST return `500`, and direct Node exceeded 10 seconds and was killed `137`; accepted deployment policy therefore preserves the image entrypoint, root `0:0`, writable ephemeral root, dropped `ALL`, no privilege escalation, and `RuntimeDefault`. This evidence is uncommitted/unpublished/unreleased web work on CI-owned `0.5.32` base commit `26fc9555ccd3ebe3593d2f353700938ec542f4fe`, not a release or Kubernetes POC.
