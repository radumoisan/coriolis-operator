...
- The inventory, pull, and local marker-plus-four foundational runtime gates are **complete**. The current policy and implemented runtime boundary are recorded in the [Foundational Resource Contract](foundational-resource-contract.md); Services, Ingress, workloads, readiness, and remaining runtime design are deferred, and no Coriolis core runtime workloads have been implemented or deployed.
