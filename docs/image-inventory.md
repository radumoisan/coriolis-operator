...
### :material-application-edit-outline: Scheduler, Transfer-Cron, And Minion-Manager Runtime Qualification

The tracked validator qualifies the exact `2603.4` scheduler image `sha256:45bea9e...a73c41`, transfer-cron image `sha256:3a44d3b...2b3ea9c`, and minion-manager image `sha256:1ea016dd...e1fb9` under their exact direct commands. All run as numeric UID/GID `42434` with read-only root, dropped `ALL`, no-new-privileges, read-only generated `/etc/coriolis`, writable only `/tmp` and `/var/log/coriolis`, on a private internal network with no host ports and no Service/listener. Scheduler is released as `0.5.26`; transfer-cron is released as `0.5.27` by CIXpress `ermyhd`; minion-manager source `5294df9a50e6c8a15c6cad374c66c3a9ab95b077` is released as `0.5.28` by CIXpress `f3pgh0`, with operator imageID `sha256:4e1a90f2bb883f80437ca048aa6645a9e94d87b80f2f82ad1a4964993e88886b`. The accepted released-artifact POC proved exact minion-manager digest/command/security, fixed-output diagnostics, empty-pool MariaDB read, stability, Pod replacement, RabbitMQ recovery, retained no-write CR recreation, and normal cleanup. These lifecycle results do not change the frozen digest table above.
