...
The tracked validator qualifies the exact `2603.4` scheduler image `sha256:45bea9e...a73c41` and transfer-cron image `sha256:3a44d3b...2b3ea9c` under their exact direct commands; both run as numeric UID/GID `42434` with read-only root, dropped `ALL`, no-new-privileges, read-only generated `/etc/coriolis`, writable only `/tmp` and `/var/log/coriolis`, on a private internal network with no host ports and no Service/listener. Post-review hardening rejects declared image ports/volumes/healthchecks and unexpected runtime mount/tmpfs targets; the strengthened rerun passed 85 stages with 0 failures and `SUMMARY runtime passed 305.963`, including complete cleanup. Scheduler source `d4805ea` is now released as operator/chart `0.5.26`, and its isolated Kubernetes POC accepted the same exact scheduler digest, live restricted contract, replacement, RabbitMQ recovery, retained no-write recreation, and normal cleanup. Transfer-cron remains qualification-only and unimplemented with no Kubernetes manifest. These lifecycle results do not change the frozen digest table above.
