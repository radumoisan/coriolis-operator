...
The tracked validator qualifies the exact `2603.4` scheduler image `sha256:45bea9e...a73c41` and transfer-cron image `sha256:3a44d3b...2b3ea9c` under their exact direct commands; both run as numeric UID/GID `42434` with read-only root, dropped `ALL`, no-new-privileges, read-only generated `/etc/coriolis`, writable only `/tmp` and `/var/log/coriolis`, on a private internal network with no host ports and no Service/listener. Scheduler is released as `0.5.26`; transfer-cron source `293500a0a64f6e10c776d674c141ae1ea3ba05e4` is released as `0.5.27` by CIXpress `ermyhd`, with released operator imageID `sha256:7c70f0e1c54559440d02d5595b054c48c7b0d586f9f61b86aeaf717364b104cf`. Its accepted isolated POC proved the same transfer digest/live restricted contract, collision auto-recovery, empty schedule diagnostics, conductor/transfer/RabbitMQ replacement, retained no-write recreation, and normal cleanup. The pre-release validator result `SUMMARY runtime passed 320.708` remains historical evidence. These lifecycle results do not change the frozen digest table above.
