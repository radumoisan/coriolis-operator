...
The adaptor image's source-to-image provenance is incomplete because its published OCI metadata lacks a source revision label; that limitation remains. The stack was qualified from released operator `0.5.49`; the explicit residuals are closed and Milestone 8 is accepted and closed: `0.5.49` (source `e59833c`, CIXpress Default `wcx6h3` all expected steps `SUCCEEDED`, CI commit `40c0f6d`, operator digest `sha256:68eacc65ce877065d6850c7888eb06a3a09f15254da206ed8168851fab31d44e`) proved ingestion, cross-CR isolation, bounded history/retention, replay/streaming, fixed-output token safety, retention-expiration handling, and cleanup. The frozen image identities below are used by that qualified stack.

The local `coriolis-logs-api-adaptor` gate passed 166 tests, Ruff lint/format, strict mypy over 10 source files, and a non-root read-only-root container startup. Ephemeral image `3e46bf5e48ce` was removed after verification. In the original 2026-08-29 released qualification, physical old-tenant retention-file deletion was not directly observed; only logical expiration and the clean compactor pipeline were proven. The second public TLS endpoint residual is closed: a controlled DNS focused rerun (context `virt-infra-dev-buc-hq`/`default`, namespace/Application `coriolis-logging-tls-validation-20260830`, released `0.5.49` with `skipCrds`, host `coriolis-logging-isolation.app.cloudbase.wiki`) passed default-trust checks with exact HTTP `308`, hostname/SAN and Let's Encrypt chain, unauthenticated `/logs` `401`, authorized `/logs` `200`, and WSS `/log-stream` `101` with a valid frame, with no insecure TLS and no credential/token value printed; cleanup was normal. Formal follow-up closed physical retention-file deletion, the 2026-08-31 strict value-silent qualification closed live backend-tail reconnect end-to-end, and the 2026-09-03 one-shot strict formal qualification completed the whole run end-to-end value-silently (`SUMMARY logging-formal passed 11602.859`, process exit 0), closing the final whole-run credential-safety residual. There are no digest or image changes.

The physical-deletion wording above describes the original 2026-08-29 qualification. Follow-up runs close that residual: value-silent `scripts/validate-coriolis-retention-runtime.py` passed 58 focused tests and Ruff; the shortened released-digest diagnostic proved 12 persisted candidates later physically absent; and the untouched-release formal run proved all 12 old-prefix candidates absent after the exact `1h`/`15m`/`2h` retention/delete window while retained Secret/PVC identities and credential values stayed unchanged without emission. The formal run's global deletion-marker count was 2, non-gating and not attributed to old candidates. The 2026-08-31 strict value-silent qualification then closed live backend-tail reconnect end-to-end, and the 2026-09-03 one-shot strict formal qualification completed the whole run end-to-end value-silently, closing the final whole-run credential-safety residual. There are no digest or image changes.
...
### :material-application-edit-outline: Barbican Selection Evidence For ADR 0008

Milestone 9 selects the operator-managed per-appliance Barbican frozen in [ADR 0008](decisions/0008-operator-managed-barbican.md). All three pinned `2023.1-ubuntu-jammy` Barbican images above are Kolla `16.6.1` carrying Barbican `16.0.2`, default to user UID/GID `42403` with supplemental `kolla` group `42400`, and were included in the historical 21-reference pull validation. The selected runtime surface is API and worker only: the API serves uWSGI on port `9311` and the worker command is `barbican-worker`. The keystone-listener image remains mirrored inventory only: it handles Keystone project-deletion cleanup, which is outside the bounded endpoint CRUD scope, so it is omitted. There is no Barbican PVC: `simple_crypto` keeps encrypted per-project key metadata and payload state in MariaDB under a static 32-byte URL-safe base64 `kek` configuration value in the retained credentials Secret, and the insecure built-in default is forbidden. The architecture is accepted and the source implementation is complete locally. Released operator `0.5.52` was qualified in isolation on 2026-09-04 and failed before the runtime validator and browser because the API WSGI app ignored uWSGI `pyargv`, searched default oslo config paths, and fell back to SQLite; the locally validated startup fix mounts the projected `barbican.conf` at the canonical default `/etc/barbican/barbican.conf` in the API main container and passed the corrected local gate. The corrected release `0.5.53` (corrective source `1a0ff21fb74896f76dce879ed021c5cebb20b4ed`, CI release commit `18859efd2e15435448b04e94cc31ba6553bef3ac`, exact isolated operator digest `sha256:418c7036b4f155feb383a2f9771d18846f8a0d35f40d678dfc9de8e7fd3ba767`) then passed isolated live qualification in `coriolis-barbican-validation-20260906` — 17/17 appliance workloads plus the isolated operator Ready, the pinned API and worker Running/Ready zero-restart with the exact `/etc/barbican/barbican.conf` read-only subPath mount observed — and the runtime validator and actual-browser two-endpoint flow closed, so Milestone 9 is accepted and closed for the bounded endpoint credential/browser CRUD scope.

### :material-application-edit-outline: Milestone 10 Actual-Web Qualification Evidence

Milestone 10 qualified the bounded actual-Web two-endpoint `live_migration` POC on released chart/operator `0.5.54` (CI release commit `b486c2611384eaff95dd4e4aa0c42efa4cc87628`, isolated operator digest `sha256:e41f5560f7e25435df175e355fc6bd9d3e49872175cdf23ef110350cdfe8fbbf`) using the unchanged frozen application ledger: the pinned immutable `coriolis-web:2603.4` image `sha256:32ebc391ac46fe627185694b3fd252afd7587b152f526dff38ae0a5b887c0db1` served the real browser flow in `virt-infra-dev-buc-hq` namespace/Application `coriolis-m10-20260906-153410` (appliance `ui-live-migration`, host `coriolis.app.cloudbase.wiki`), where 17/17 appliance workloads plus the isolated operator were Ready with zero restarts and public hostname-valid TLS returned HTTP 200. This observation evidence does not change the frozen digest tables above.

### :material-application-edit-outline: Milestone 11A Logging-Hardening Deployment Evidence

Milestone 11A logging hardening was initially published from source `f0c688bb835b7b80df6e9a803c3acaeda34bdc3b` by CIXpress Default `zom4hw` (CI release commit `6a22a270f312e2f2230829025b56a08e5bdf039f`, chart/app/operator `0.5.56`) against the unchanged frozen `2603.4` application ledger, and deployed isolated in `virt-infra-dev-buc-hq` namespace/Application `coriolis-m11a-20260908` (appliance `logging-hardening`, host `coriolis.app.cloudbase.wiki`) at observed operator digest `sha256:91e0566b80dcfbdfbaa61607adbc00c3a742fd49fe7abab51f2e9e3412e77d40` with 19 zero-restart Pods. Live observation produced one image-behavior finding: the immutable `coriolis-conductor:2603.4` image (`sha256:27495f44fbb8b320098d0aa04cd9dcb2a4b432e57aa17417606efc5403ac09c7`) declares its `temp_keypair_password` oslo option without `secret=True`, so under `debug=true` the credential value reaches conductor stdout/stderr and then Pod logs (proven value-silently; no value, hash, or encoded form recorded), which drove the user-selected `coriolisDebug` safe-default change to false. The selector and safe-default corrections were published from exact pipeline head `56bd8b78d2cdd51fedd1a8f191ab08104323008e` by CIXpress Default `qk93o3` as CI release commit `eac5029e6ef546ed0831be74476738e7e7fbc1b0`, chart/app/operator `0.5.57`; the isolated runtime used exact operator digest `sha256:b58de051df3bf8060566867f6da2fc7c514baf24652d3f45234c9547bd706db1`, the released natural observer passed all 14 producers with `debug=false`, formal `1h`/`1m`/`5m` retention passed with physical deletion, and normal cleanup completed without shared mutation. Milestone 11A is accepted and closed for this bounded logging-hardening POC on `0.5.57`; explicit `coriolisDebug: true` remains unsafe, broader Milestone 11 stays open, and the frozen application digest tables above are unchanged.
