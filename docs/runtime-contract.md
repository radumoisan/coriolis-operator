...
Clean first boot of `0.5.5` exposed anonymous initialization accounts shadowing `coriolis@%`; commit `3ee5d2d` adds `--skip-test-db` and CIXpress released it as `0.5.6`. Released `0.5.6` passed clean single-node `local-path` validation without repair: RWO Filesystem provisioning, fsGroup writes, exact retained reuse, persisted state, authenticated probes, same-node remount, and normal 12-second termination. RabbitMQ `0.5.11` has accepted released-artifact single-node POC evidence. Keystone exact-image standalone evidence and reconciliation implementation now cover database, schema, retained keys, bootstrap, direct WSGI, authenticated HTTP, restart, collision-safe preflight, ordered writes, and cleanup; the implementation is not released, deployed, or POC-tested in Kubernetes. CSI and cross-node attach/detach remain unvalidated, and backup/restore, HA, RPO/RTO, credential/key rotation, and production storage remain open. Remaining Coriolis-common bootstrap/readiness, Barbican and other Services, and route emission remain later milestones. No Ingresses or Jobs are implemented.
...
Milestone history remains: image/runtime inventory, the MariaDB single-node POC, the Memcached `0.5.8` POC, the RabbitMQ `0.5.11` accepted POC, and Keystone standalone evidence are complete. Keystone acceptance here is local only: dedicated database/user, schema, retained-key model, idempotent bootstrap, direct WSGI, authenticated HTTP, restart, and cleanup passed in `69.063s`; reconciliation is now implemented and validated with 397 tests plus repository quality gates. No appliance or runtime workload remains after cleanup. MariaDB/RabbitMQ CSI cross-node evidence, backup/restore, HA, RPO/RTO, credential/key rotation, and production storage remain later gates. A CI-owned Keystone release and released-artifact POC are next.
...
5. RabbitMQ is published and its isolated released-artifact POC is accepted. Keystone standalone evidence and no-PVC/no-Job reconciliation are complete; release it through CI and run an isolated released-artifact POC before Coriolis-common bootstrap.
