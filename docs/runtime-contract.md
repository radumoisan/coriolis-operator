...
4. Generated configuration and secrets. *(Pure builders are committed at `050f16e`, pure retained credential generators at `a604579`, pure retained Secret semantic validation/extraction at `5165629`, non-sensitive configuration rendering at `97153a7`, and five-resource preflight at `35eac9b`, none pushed/deployed. The preflight uses `OwnedClassification.ABSENT/MANAGED/COLLISION` and classifies metadata for all five frozen resources before retained Secret semantics; semantic failures map fail-closed to that resource's `COLLISION`, and corresponding generators run only for `ABSENT` retained Secrets after collision-free validation. Returned credentials remain internal and value-safe. Next: runtime Kubernetes pre-reads before mutation, only required Secret/ConfigMap `get` and SSA RBAC, SSA application, and reconciliation/status collision/failure semantics; retained adoption/runtime construction and rotation remain deferred.)*
