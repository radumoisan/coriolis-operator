...
3. Foundational resource contracts. *(documented in the [Foundational Resource Contract](foundational-resource-contract.md); metadata, collision, retained-resource classification, manifest builders, credential generation, retained Secret semantics, non-sensitive rendering, and pure five-resource preflight are implemented in local unpushed/undeployed slices through `35eac9b`. The documentation-only sensitive renderer contract is committed at `574efcf`, but its pure implementation does not yet exist. Before any MariaDB vertical slice, the remaining sequence is sensitive renderer implementation/tests, multi-resource read/apply failure/status/atomicity/marker-last policy, then runtime pre-reads/RBAC/SSA. Decoded values remain internal and must never be logged, statused, or evented. TLS/optional credentials/storage/probes/readiness/bootstrap/rotation remain deferred.)*
4. Generated configuration and secrets. *(Pure builders are committed at `050f16e`, pure retained credential generators at `a604579`, pure retained Secret semantic validation/extraction at `5165629`, non-sensitive configuration rendering at `97153a7`, five-resource preflight at `35eac9b`, and the documentation-only sensitive renderer contract at `574efcf`, none pushed/deployed. The complete `coriolis.conf` remains exactly one key in the owner-referenced configuration Secret, never ConfigMap/log/status/event/metadata/documentation content. The frozen future pure renderer uses the immutable upstream base template and all 16 fragments with preserved provider lists/order/module maps, no initial custom module overrides or compression/compressor, explicit internal dependency inputs, source-audited identities/paths, credential mappings, strict value-safe validation, and no CRD fields. All three generated credential Secrets use exactly `coriolis.cloudbase.it/retention: state-credentials`. Source review covered every base-template variable and all fragments; `git diff --check` passed. Next: implement/test the pure sensitive renderer; freeze multi-resource read/apply failure, status, atomicity, and marker-last semantics; then runtime five-resource pre-reads, minimal RBAC, and SSA. Runtime integration is not currently unblocked; retained adoption/runtime construction and rotation remain deferred.)*
