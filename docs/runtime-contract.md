...
The development stack through MariaDB reconciliation is published on `origin/dev` at `55212b0` and included in observed operator `0.5.5`. It retains the read prefix and implements exactly four Services in frozen order: RabbitMQ `5672`, Memcached `11211`, MariaDB `3306`, and Keystone `5000`. It uses ordered reads, completes preparation before writes, returns mutation-free `InvalidRuntimeConfiguration` for invalid storage/resources configuration, creates an absent PVC or exactly reuses it without writes, uses guarded SSA for managed ConfigMap, Secret, and StatefulSet resources, and writes MariaDB resources before the marker. PVC RBAC is `get`/`create`; StatefulSet RBAC is `get`/`create`/`patch`.

Single-node `local-path` POC validation proved RWO Filesystem provisioning, fsGroup writes, exact retained reuse, persisted state, authenticated probes after controlled repair, and termination within 30 seconds. Clean first boot of `0.5.5` exposed anonymous initialization accounts shadowing `coriolis@%`; the local `--skip-test-db` fix is validated by focused tests but is not released or clean-first-boot validated. CSI and cross-node attach/detach remain unvalidated. Remaining dependency bootstrap, probe, and readiness work, Barbican and other Services, and route emission remain later milestones. No endpoints, Ingresses, or Jobs are implemented.
...
Milestone history remains: image/runtime inventory is complete and the development stack through MariaDB reconciliation is published on `origin/dev` at `55212b0`. Operator `0.5.5` is observed in `coriolis`, but no appliance or runtime workload remains after POC cleanup. Publishing and clean-first-boot validating the anonymous-account fix is next; CSI/cross-node evidence and production storage acceptance remain later gates. Remaining Services and Ingress routes follow only after their backends are defined. The first runtime acceptance is complete bootstrap with internally healthy UI/API, not a migration test.
