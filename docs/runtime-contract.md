...
4. Generated configuration and secrets. *(Pure builders are committed at `050f16e`, pure retained credential generators at `a604579`, pure retained Secret semantic validation/extraction at `5165629`, and non-sensitive configuration rendering at `97153a7`, none pushed/deployed. `render_coriolis_config` validates explicit inputs and renders exactly the six frozen ConfigMap files through Jinja2 `PackageLoader` with `StrictUndefined`, disabled autoescape, and preserved trailing newlines; six verbatim Apache-2.0 upstream templates are packaged with source/license attribution. It renders no Secret content. Future preflight for all five frozen resources classifies metadata first, maps retained Secret semantic failure fail-closed to `COLLISION`, invokes generators only for `ABSENT`, and keeps returned decoded values internal, never logged/statused/evented. Then: SSA, minimal Secret RBAC, and reconciliation/status failure semantics; rotation remains deferred.)*
