...
1. **Multi-resource runtime read/apply failure, reconciliation status, atomicity, and marker-last semantics**, followed by collision-safe Kubernetes pre-reads for all five frozen resources before mutation, minimal Secret/ConfigMap `get` and SSA RBAC, and SSA application. The pure builders, generators, semantic helper, non-sensitive renderer, preflight, and sensitive renderer are committed locally at `050f16e`, `a604579`, `5165629`, `97153a7`, `35eac9b`, and `9bb20f3`; decoded values remain internal and must never be logged, statused, or evented.
