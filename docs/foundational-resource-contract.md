...
The development stack through MariaDB reconciliation is published on `origin/dev` at `55212b0` and included in observed operator `0.5.5`. It performs ordered reads and all preparation before writes, returns stable mutation-free `InvalidRuntimeConfiguration` for invalid storage/resources configuration, creates an absent PVC or exactly reuses it without writes, uses guarded SSA for managed ConfigMap, Secret, and StatefulSet resources, and writes MariaDB resources before the marker. The accepted single-node POC result and remaining clean-first-boot/CSI limits are recorded below. `Ready=False/RuntimeNotImplemented` remains truthful until runtime readiness is implemented.
...
The immutable upstream appliance source still contains Step CA and web-proxy roles. They are historical source evidence, not initial Kubernetes runtime selection. The development stack through MariaDB reconciliation is published on `origin/dev` at `55212b0` and included in observed operator `0.5.5`. It includes ordered reads, all preparation before writes, stable mutation-free `InvalidRuntimeConfiguration`, exact PVC create/no-write reuse, guarded ConfigMap/Secret/StatefulSet SSA, MariaDB writes before the marker, PVC `get`/`create`, and StatefulSet `get`/`create`/`patch` RBAC. Validation passes with 316 tests, Ruff lint/format, mypy, Helm lint/template, and `git diff --check`. The accepted single-node POC result and remaining clean-first-boot/CSI limits are recorded below; production backup/restore, HA, and RPO/RTO remain open. `Ready=False/RuntimeNotImplemented` remains truthful.
...
- The single-node `local-path` POC proved RWO, fsGroup, exact retained reuse, persisted same-node remount, and clean 30-second termination after a controlled anonymous-account repair. The local `--skip-test-db` fix still requires released clean-first-boot validation; CSI rescheduling/attach-detach remains unvalidated. Production is still blocked on backup/restore, HA, and RPO/RTO.
...
The development stack through MariaDB reconciliation is published on `origin/dev` at `55212b0` and included in observed operator `0.5.5`. It includes the marker API/migration, metadata, retained-resource classifier, builders, credential generation, retained Secret validation, renderers, preflight, Kubernetes render-input factory, derived template variants, ingress schema/resolver, marker-plus-four runtime gate (`862777d` with status `f219977`), four-Service slice (`797235b`), and MariaDB pure desired-state preparation (`5a2dfce`). Current validation passes with 316 tests, Ruff lint/format, mypy, Helm lint/template, and `git diff --check`.

For MariaDB, the development contract fixes workload, retained storage, generated manifests, bootstrap, probes, resource rules, lifecycle, and console logging; preparation and reconciliation are published on `origin/dev` through `55212b0` and included in observed operator `0.5.5`. The single-node `local-path` POC validated RWO, fsGroup, retained reuse, same-node remount, and clean termination after controlled repair, while clean first boot exposed anonymous accounts now prevented by the local `--skip-test-db` change. A released clean-first-boot rerun and later CSI/cross-node evidence remain open. Production remains blocked on backup/restore, HA, and RPO/RTO acceptance. Keystone schema/fernet/bootstrap evidence and a selected, proven `coriolis-dbsync` Job contract remain unresolved. Credential rotation, provider private material, optional component credentials, Barbican and other backend Services, and Ingress routes after backend workloads remain deferred.
...
- MariaDB pure desired-state preparation and reconciliation are published on `origin/dev` through `55212b0` and included in observed operator `0.5.5`; current validation is 316 tests plus Ruff lint/format, mypy, Helm lint/template, and `git diff --check`. Publish and clean-first-boot validate the local anonymous-account fix next; CSI/cross-node and production backup/restore, HA, and RPO/RTO remain open.
