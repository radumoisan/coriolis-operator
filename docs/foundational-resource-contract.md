...
This page freezes the foundational Kubernetes resource contract for the `core` profile. It records policy, authoritative evidence, implemented pure/API-local slices, and runtime slices. Argo currently runs operator `0.5.11`, `1/1` with zero restarts and zero appliance CRs; that deployed controller does not establish overall appliance readiness.
...
No future route may be emitted until its backend Service exists. RabbitMQ, Memcached, MariaDB, and Keystone Services are implemented. MariaDB, Memcached, and RabbitMQ workloads are released and POC-tested, but no workload is currently deployed after cleanup; Keystone is implemented but not released, deployed, or POC-tested. Application workloads, endpoints, Ingress, and other Services are not implemented. Before adding the web workload, prove offline that the web image starts without `CA_FINGERPRINT` and without a Step CA mount.
...
This section began as the documentation-only dependency evidence gate. MariaDB and Memcached have released implementations and accepted single-node POCs; RabbitMQ is released in `0.5.11` with accepted single-node POC evidence. Keystone standalone evidence and implementation are complete, while its CI-owned release and released-artifact POC acceptance remain open. Later dependencies remain fail-closed on missing evidence. The contracts below distinguish implementation, release, POC, and production readiness.
...
The mirrored tags were pull-validated. MariaDB, Memcached, and RabbitMQ workloads additionally validated their exact tag-plus-digest identities in accepted single-node POCs. Local Keystone validation now covers the exact digest, direct standalone runtime, database schema, key setup, bootstrap, authenticated HTTP, restart behavior, and operator implementation, but not a release or Kubernetes POC. Future workload manifests must pin the approved mirror digest and use the read-only external `coriolis-appliance-registry` pull Secret.
...
- Released `0.5.11` accepted the single-node `local-path` gates: fsGroup/RWO, Service/EndpointSlice, authenticated AMQP through Service DNS, stable readiness, normal replacement/same-node persistence, exact retained no-write CR recreation, and cleanup. CSI cross-node, backup/restore, HA, RPO/RTO, credential rotation, and production storage remain later gates. Keystone standalone evidence and local implementation are now complete. `Ready=False/RuntimeNotImplemented` remains truthful.

## :material-book-open-page-variant-outline: Keystone Kubernetes Contract

This frozen development contract follows the completed standalone evidence above. It is implemented and validated, but is not released, deployed, POC-tested, HA, rotation-capable, or production-ready.
...
Complete all four dependency interfaces first. MariaDB's accepted clean single-node POC is complete; CSI/cross-node evidence remains later. Memcached is released and its isolated POC is complete. RabbitMQ `0.5.11` has accepted released-artifact POC evidence. Keystone standalone evidence, development contract, and implementation are complete; a CI-owned release and released-artifact POC follow. Coriolis common bootstrap follows healthy dependencies. Historical Ansible order is evidence, not Kubernetes readiness proof.
