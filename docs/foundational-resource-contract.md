...
The sensitive renderer is a **future pure implementation**; no renderer code or tests are added by this contract. Its only output is exactly one `coriolis.conf` key in the owner-referenced, rebuildable `<appliance>-coriolis-config-secret`. The rendered content must never be placed in a ConfigMap, metadata, status, event, log, or documentation content.
...
The future renderer must package verbatim the Apache-2.0 upstream base template `coriolis_ansible/roles/coriolis/common/templates/coriolis.conf.j2` and all provider fragments below, using Jinja2 `PackageLoader`, `StrictUndefined`, and `autoescape=False`. It must render deterministically and preserve the upstream trailing newline. Packaged sources must retain source and Apache-2.0 license attribution. This is a source-fidelity requirement, not a claim that an implementation exists.
...
!!! note
    CA path rendering is frozen, but CA material and bootstrap are deferred. The future renderer does not establish the referenced CA file.

The future implementation must use an exact typed internal schema: required non-empty strings; ports in `1..65535`; Keystone protocol restricted to `http` or `https`; and credentials required, non-empty, and value-safe. It must reject CR, LF, and NUL injection; never use secret-bearing object representations; emit only fixed category-only errors; avoid render diagnostics containing context values; avoid input mutation; and fail closed for missing, extra, or undefined values. This contract defines no CRD fields.

The future interface boundary separates non-sensitive endpoint inputs from credential inputs supplied only by successful `FoundationalResourcePreflight`. Sensitive rendered output must remain in a credential-redacted wrapper before it is composed into the existing configuration-Secret builder; this is conceptual only and does not freeze Python class or function names.
...
Non-goals: no renderer code or tests now; no runtime Kubernetes read/write, SSA, RBAC, status, CRD, dependency Service/workload, database/Keystone bootstrap, TLS/CA material, optional component credential, provider connection credential/private material, storage, probe, readiness, rotation, or deployment change.
...
- The documentation-only **sensitive renderer contract** is **committed locally at `574efcf` on `dev`, but not pushed or deployed**. It freezes the complete `coriolis.conf` as exactly one key in the owner-referenced configuration Secret; it is never ConfigMap, log, status, event, metadata, or documentation content. The frozen future pure renderer source model is the immutable upstream base template plus all 16 provider fragments, preserving the authoritative provider lists/order/module maps; custom module overrides and compression/compressor are disabled initially. Explicit internal dependency inputs, fixed source-audited identities/paths, credential mappings, strict value-safe validation, and a redacted interface boundary are frozen without CRD fields. All three generated credential Secrets have exactly `coriolis.cloudbase.it/retention: state-credentials`. Source review covered every base-template variable and all fragments; `git diff --check` passed. No renderer implementation/tests or runtime behavior changed; TLS/CA material, provider connection/private data, optional credentials, dependency Services/workloads, bootstrap, storage, readiness, and rotation remain deferred.
...
1. **Pure sensitive `coriolis.conf` renderer implementation and tests**, followed by a frozen multi-resource runtime failure, apply, status, atomicity, and marker-last policy; only then runtime collision-safe Kubernetes pre-reads for all five frozen resources before mutation, required Secret/ConfigMap `get` and SSA RBAC, and SSA application. The pure builders, generators, semantic helper, non-sensitive renderer, and preflight are committed locally at `050f16e`, `a604579`, `5165629`, `97153a7`, and `35eac9b`; decoded values remain internal and must never be logged, statused, or evented.
