...
Historical `0.5.17` acceptance remains recorded below. Release `0.5.19` is published but its POC is unaccepted: source `c89b57fcd5b2ee160c1a0986be9fedc9edc19ec8` was published by successful pipeline `mg04ql` as CI release `3a2af04270762069f3d50905ad6f4255d699c302`. Persistent collision remained correctly no-churn. Conflict deletion at `14:50:14Z` triggered timer recovery at `14:50:42Z` (28s), created bootstrap resources, and published `BootstrapRunning` through `TemporaryError`; the following Kopf retry no-oped because it evaluated only `ResourceCollision`. The Job succeeded at `14:51:42Z` with exit `0` and restarts `0`, and dependencies were healthy, but the CR remained `Reconciled=False/BootstrapRunning` for over 10 minutes. `0.5.19` is not accepted. Local, unpublished code accepts Kopf `retry` and continues normal reconciliation only for the initial exact collision or `retry > 0`; full validation passes at 490 unit tests, Ruff lint/format, mypy, Helm lint/template, container build, and `git diff --check`, while release and re-POC remain pending. `coriolis-common` remains a base image, not a workload.
...
- Local, unpublished collision recovery retains the 60-second initial-delay/interval timer. It invokes normal reconciliation only when persisted status contains the exact `Reconciled=False/ResourceCollision` condition or when Kopf supplies `retry > 0`; malformed and noncollision initial calls remain no-ops. This narrowly continues the retry that published `BootstrapRunning`, rather than broadening periodic reconciliation. Reconciliation patches only changed operator-owned computed status fields and preserves unrelated status. Full validation passes at 490 unit tests, Ruff lint/format, mypy, Helm lint/template, container build, and `git diff --check`; release and re-POC remain pending.
- Accepted collision evidence: an unmanaged immutable v2 ConfigMap collision remained exact (UID, resourceVersion, creation time, key, and hash) and returned `ResourceCollision` with zero newly owned writes. The local timer is intended to retry this narrow stable status; it adds no resource adoption, deletion, list/watch permission, CRD change, finalizer, destructive behavior, or readiness change.
...
- Local validation passes at 477 unit tests, Ruff lint/format, mypy, Helm lint/template, exact conductor parser check, Docker build, and `git diff --check`; exact renderer runtime is `135.212s`. Output-suppressed exact-image verification passed twice in 10s, including post-recreation, for schema tables, the unique enabled Coriolis user/default service project/password auth/admin assignment, migration service, and exactly one RegionOne admin/internal/public endpoint each. Final state is v2 only, v1 absent, `acceptedVersion=2603.4`, `Accepted=True`, `Reconciled=True`, `Degraded=False`, `Ready=False/RuntimeNotImplemented`. No `Ready=True`, application workload, or RBAC change is added.
