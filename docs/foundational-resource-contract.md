...
No future route may be emitted until its backend Service exists. RabbitMQ, Memcached, MariaDB, and Keystone Services are implemented. MariaDB is published and Memcached is implemented locally, but neither runtime workload is currently deployed; RabbitMQ, Keystone, and application workloads, endpoints, Ingress, and other Services are not implemented. Before adding the web workload, prove offline that the web image starts without `CA_FINGERPRINT` and without a Step CA mount.

## :material-book-open-page-variant-outline: Current Status And Accuracy

The immutable upstream appliance source still contains Step CA and web-proxy roles. They are historical source evidence, not initial Kubernetes runtime selection. The development stack through MariaDB reconciliation and anonymous-account prevention is published on `origin/dev`, with the fix released as `0.5.6`. The local Memcached implementation extends its ordered reads, all-preparation-before-writes boundary, guarded SSA, and marker-last behavior with one ephemeral Deployment and Deployment-only RBAC. Validation passes with 326 tests, Ruff lint/format, mypy, Helm lint/template, and `git diff --check`. The accepted MariaDB single-node POC and remaining CSI limits are recorded below; Memcached publication and released-artifact POC plus production backup/restore, HA, and RPO/RTO remain open. `Ready=False/RuntimeNotImplemented` remains truthful.
...
| Memcached | Service `11211`; no credentials, provisioning, or configuration found. OCI and standalone evidence support the local Deployment implementation described below. | Local implementation is uncommitted, unpushed, and undeployed; review, publication, and a released-artifact isolated POC remain required. |
...
This frozen development contract now has a matching local, uncommitted, unpushed, and undeployed implementation. It is not a cluster deployment or POC claim.
...
Complete all four dependency interfaces first. MariaDB's OCI, standalone startup/bootstrap, development Kubernetes design contract, desired-state preparation, reconciliation, and anonymous-account prevention are published on `origin/dev`, with the fix released as `0.5.6`. The accepted clean single-node POC is complete; CSI/cross-node evidence remains later. Memcached implementation is local and awaits review, publication, and a released-artifact isolated POC; RabbitMQ evidence follows after Memcached acceptance. Keystone follows MariaDB; Coriolis common bootstrap follows healthy dependencies. Historical Ansible order is evidence, not Kubernetes readiness proof.
...
The development stack through MariaDB reconciliation is published on `origin/dev` at `55212b0` and included in released operator `0.5.6`. It includes the marker API/migration, metadata, retained-resource classifier, builders, credential generation, retained Secret validation, renderers, preflight, Kubernetes render-input factory, derived template variants, ingress schema/resolver, marker-plus-four runtime gate (`862777d` with status `f219977`), four-Service slice (`797235b`), and MariaDB pure desired-state preparation (`5a2dfce`). The local Memcached Deployment implementation is uncommitted, unpushed, undeployed, and absent from released `0.5.7`; its validation passes with 326 tests, Ruff lint/format, mypy, Helm lint/template, and `git diff --check`.
