...
This page freezes the foundational Kubernetes resource contracts for the `core` runtime profile: deterministic naming, metadata labels, ownership/deletion/retention, Secret/configuration classes, credential-generation policy, retained Secret semantics, non-sensitive configuration rendering, and a dependency evidence inventory with a proposed implementation ordering. It records **policy and evidence**, not runtime implementation. Metadata-only helpers, the pure retained-resource authorization/classification slice (`classify_retained_resource`), five pure Secret/ConfigMap manifest builders, three pure retained credential generators, the pure `validated_retained_secret_values` helper, and pure `render_coriolis_config` are implemented locally; no Kubernetes reconciliation, read/SSA/RBAC path, dependency, or workload construction/deployment is implemented, and nothing here changes deployed runtime behavior.

!!! note
    This is the first Milestone 4 deliverable, a documentation-only contract slice plus implemented local helper/classifier/builder/generator/semantic-validation/rendering slices. The deployed operator `0.5.3` remains marker-only; the local API slice is committed at `ab9df83`, the metadata-only helper slice at `fbab6e5`, the collision/migration marker API-layer slice at `d8df00f`, the retained-resource authorization/classification slice at `1b73045`, the documentation-only Secret/configuration contract slice at `8ce26ba`, the pure builder slice at `050f16e`, the pure generator slice at `a604579`, the semantic validation/extraction slice at `5165629`, and the non-sensitive renderer at `97153a7`, all on `dev` but not pushed or deployed. A MariaDB vertical slice still follows five-resource collision-safe preflight, SSA, minimal Secret RBAC, reconciliation/status failure semantics, storage, probes/readiness/bootstrap, and rotation decisions.
...
The following are intentionally out of scope for runtime implementation and are not decided here: full dependency/workload construction, retained-resource preflight/reconciliation/RBAC, status failure semantics, cluster validation, and Kubernetes Job design. The pure Secret/ConfigMap builders are committed locally at `050f16e`, the pure credential generators at `a604579`, semantic validation/extraction at `5165629`, and non-sensitive rendering at `97153a7`; the foundational names, key layouts, operator-generated-only policy, generation algorithm, persisted retained Secret semantics, and six non-sensitive ConfigMap-file rendering boundary are frozen above. Recorded ports, commands, probes, volumes, storage sizes, and readiness checks are **not guessed** here; they are listed as unresolved and require authoritative evidence.
...
1. **Collision-safe preflight for all five frozen resources before mutation; metadata classification before retained Secret semantic validation, with semantic failure mapped fail-closed to `COLLISION`; `ABSENT`-only credential generation; SSA application; minimal Secret RBAC; and reconciliation/status failure semantics** for the frozen Secrets and ConfigMap above. The pure builders, generators, semantic helper, and non-sensitive renderer are committed locally at `050f16e`, `a604579`, `5165629`, and `97153a7`; decoded values remain internal and must never be logged, statused, or evented.
