...
This page freezes the foundational Kubernetes resource contracts for the `core` runtime profile: deterministic naming, metadata labels, ownership/deletion/retention, Secret/configuration classes, credential-generation policy, retained Secret semantics, and a dependency evidence inventory with a proposed implementation ordering. It records **policy and evidence**, not runtime implementation. Metadata-only helpers, the pure retained-resource authorization/classification slice (`classify_retained_resource`), five pure Secret/ConfigMap manifest builders, three pure retained credential generators, and the pure `validated_retained_secret_values` helper are implemented locally; no Kubernetes reconciliation, read/SSA/RBAC path, dependency, or workload construction/deployment is implemented, and nothing here changes deployed runtime behavior.

!!! note
    This is the first Milestone 4 deliverable, a documentation-only contract slice plus implemented local helper/classifier/builder/generator/semantic-validation slices. The deployed operator `0.5.3` remains marker-only; the local API slice is committed at `ab9df83`, the metadata-only helper slice at `fbab6e5`, the collision/migration marker API-layer slice at `d8df00f`, the retained-resource authorization/classification slice at `1b73045`, the documentation-only Secret/configuration contract slice at `8ce26ba`, the pure builder slice at `050f16e`, the pure generator slice at `a604579`, and the semantic validation/extraction slice at `5165629`, all on `dev` but not pushed or deployed. A MariaDB vertical slice still follows non-sensitive configuration rendering inputs/templates, collision-safe pre-reads, metadata classification plus fail-closed semantic validation, `ABSENT`-only generation, SSA, minimal Secret RBAC, reconciliation/status failure semantics, storage, probes/readiness/bootstrap, and rotation decisions.
...
The following are intentionally out of scope for runtime implementation and are not decided here: full dependency/workload construction, non-sensitive configuration rendering inputs/templates, retained-resource preflight/reconciliation/RBAC, status failure semantics, cluster validation, and Kubernetes Job design. The pure Secret/ConfigMap builders are committed locally at `050f16e`, the pure credential generators at `a604579`, and semantic validation/extraction at `5165629`; the foundational names, key layouts, operator-generated-only policy, generation algorithm, and persisted retained Secret semantics are frozen above. Recorded ports, commands, probes, volumes, storage sizes, and readiness checks are **not guessed** here; they are listed as unresolved and require authoritative evidence.
...
1. **Non-sensitive configuration rendering inputs/templates; collision-safe pre-reads for every frozen resource before mutation; metadata classification before retained Secret semantic validation, with semantic failure mapped fail-closed to `COLLISION`; `ABSENT`-only credential generation; SSA application; minimal Secret RBAC; and reconciliation/status failure semantics** for the frozen Secrets and ConfigMap above. The pure builders, generators, and semantic helper are committed locally at `050f16e`, `a604579`, and `5165629`; decoded values remain internal and must never be logged, statused, or evented.
