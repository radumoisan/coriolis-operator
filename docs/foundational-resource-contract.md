...
Fix source `764b9952d1e1c9bc1cbce08afddea8781f391f42` (`Fix Coriolis-common bootstrap dbsync`) was published as accepted `0.5.17` by pipeline `ectoq4`, `2026-08-23T12:16:43Z`-`12:18:06Z`, with `git-clone`, `kaniko-build`, `helm-update`, and `cleanup` all `SUCCEEDED`; CI commit is `6bfc494d50949b7a5aa770c4febb7c5100b3b363` and operator imageID is `sha256:443e6e5dec8cd6e7f2040ca4fe1f5dcfcfa40ad36184cb30dc54a4be7547d8a6`. Historical `0.5.16` v1 failed before dbsync on provider-continuation parsing and correctly remained `BootstrapFailed`, markerless, and unmodified. `coriolis-common` remains a base image, not a workload.
...
- Local, unpublished collision recovery adds a Kopf timer with a 60-second initial delay and 60-second interval. It invokes normal reconciliation only when persisted status contains the exact `Reconciled=False/ResourceCollision` condition; malformed and noncollision status are no-ops. This is bounded recovery for a removed conflict, not a child watch or broad periodic reconcile. Reconciliation patches only changed operator-owned computed status fields and preserves unrelated status. Full local validation passes at 489 unit tests, Ruff lint/format, mypy, Helm lint/template, container build, and `git diff --check`; released-artifact POC remains pending.
