...
Released `0.5.35` source `016a25744a7c946a02fa6d89e38fa39e24e32146` reconciles three owner-referenced `networking.k8s.io/v1` Ingresses after API and web Service/Deployment: `<appliance>-coriolis-web`, `<appliance>-keystone`, then `<appliance>-coriolis-api`, before the marker. It validates settings before API creation, pre-reads/classifies/preflights all three before mutation, fails collisions all-or-nothing without mutation, and uses resourceVersion-guarded SSA for managed repair. Failures are sanitized and skip later writes/marker; `spec.ingress` changes reconcile. Apply order remains source/unit-tested; fine-grained live `ADDED` ordering was not proven. RBAC adds only Ingress `get`/`create`/`patch`. The accepted isolated POC proved deterministic-name collision holds, API-fixture-only recovery, exact manifests/routes/rewrites/owner references, trusted LetsEncrypt TLS without TLS-Secret ownership, public status checks, exact-origin CORS including `Authorization`, resume-triggered same-UID managed repair, CR recreation retained reuse/ownership transfer, and normal cleanup. Managed drift repairs on normal reconciliation, not a broad successful-CR periodic loop; the timer is collision-only and the marker is not later-drift protection. Host uniqueness across CRs is an external deployment precondition and is not enforced with list-free RBAC; multi-CR route/certificate conflicts, browser login, valid-credential provider/write workflows, HA, production storage, backup/restore, and `Ready=True` remain unvalidated. See [ADR 0006](decisions/0006-kubernetes-network-ingress.md).
...
- **Milestone 11A extension (released `0.5.57` qualified, Milestone 11A accepted and closed; the bullets above remain unchanged history).** [ADR 0009](decisions/0009-log-sink-rotation-retention-ownership.md) makes Kubernetes stdout/stderr the application sink with node-local rotation owned by the kubelet/container runtime and query retention/compaction/deletion owned by Loki. `spec.logging` additionally accepts optional `compactionIntervalMinutes` (default 15, integer >= 1), `retentionDeleteDelayMinutes` (default 120, integer >= 1), and `coriolisDebug` (safe default false shipped from released `0.5.57`; the released `0.5.56` shipped default true, proven unsafe for value-safe acceptance against the immutable `2603.4` images); compaction may not exceed the retention period while delete delay may exceed retention, and invalid values fail closed into the mutation-free `InvalidRuntimeConfiguration` status. The Kubernetes `coriolis.conf` is stderr-only (`log_dir =` empty, `use_syslog = false`, `use_stderr = true`, `debug` from `coriolisDebug`), the in-container `/var/log/coriolis` log mounts are removed from core workloads, and the worker receives a dedicated `emptyDir` at `/var/log/coriolis/vmware-root` for VixDiskLib. All 14 natural producers require useful correlated records across pod logs, Loki, and the adaptor; memcached is the readiness/readability non-empty-log exception; the logging infrastructure is self-excluded but readiness/readability audited; synthetic markers stay bounded to qualification probes. The isolated `coriolis-m11a-20260908` deployment passed the deployed-shape checks; the observer-selector and default-false corrections (`dd3530d659896b2ce806920ee6681a65256c82a0`, factual recovery commit `56bd8b78d2cdd51fedd1a8f191ab08104323008e` at the exact head) were then published by CIXpress Default `qk93o3` as release `0.5.57` (CI release commit `eac5029e6ef546ed0831be74476738e7e7fbc1b0`), and on the isolated `0.5.57` upgrade the released natural-logs observer passed all 14 producers value-silently in about 58.4s with `debug=false` and formal retention qualification passed in 4113.801s with physical deletion, followed by successful normal cleanup. Milestone 11A is accepted and closed for its bounded logging-hardening POC; broader Milestone 11 stays open.
...
## :material-book-open-page-variant-outline: Barbican Resource Contract (0.5.52 Qualification Failed On API Startup Config; Corrected Release 0.5.53 Qualified And Milestone 9 Closed)

[ADR 0008](decisions/0008-operator-managed-barbican.md) freezes the Milestone 9 per-`CoriolisAppliance` Barbican naming, credential, ownership, and bootstrap boundary. The source implementation is complete locally and passes the local gates. Release `0.5.52` was deployed for isolated qualification and failed on the API startup configuration defect recorded below; the locally validated startup fix passed the corrected local gate, and the corrected release `0.5.53` (corrective source `1a0ff21fb74896f76dce879ed021c5cebb20b4ed`, CI release commit `18859efd2e15435448b04e94cc31ba6553bef3ac`, exact isolated operator digest `sha256:418c7036b4f155feb383a2f9771d18846f8a0d35f40d678dfc9de8e7fd3ba767`, CIXpress Default `m2ah6a` with top-level and all expected steps `SUCCEEDED`) then passed isolated live qualification in namespace/Application `coriolis-barbican-validation-20260906` — `17/17` appliance workloads plus isolated operator Ready, zero-restart API/worker with the exact `/etc/barbican/barbican.conf` read-only subPath mount observed, all 18 fixed-output runtime-validator stages passing value-silently, and the actual-browser two-endpoint `secret_ref` creation/validation/selection/removal flow closing to the zero baseline — and Milestone 9 is accepted and closed for the bounded endpoint credential/browser CRUD scope.

- Retained ownerless credential Secret `<appliance>-barbican-credentials` with exactly `barbican_database_password`, `barbican_keystone_password`, and `barbican_crypto_key`, generated absent-only under the existing retained-credential policy; the existing infrastructure/coriolis Secret keysets are not altered. `barbican_crypto_key` is the `simple_crypto` master `kek`: a static 32-byte URL-safe base64 value, with the insecure built-in default forbidden. Encrypted per-project key metadata and payload state persist in MariaDB; there is no Barbican PVC.
- Owner-referenced rebuildable configuration: one ConfigMap with the non-sensitive API paste/vassal/policy/bootstrap assets and one config Secret with the rendered `barbican.conf`. Following the released `0.5.52` qualification failure — where the API WSGI app ignored uWSGI `pyargv`, searched default oslo config paths, and fell back to SQLite — the API main container additionally mounts that existing projected config Secret via subPath at the canonical Kolla/oslo default `/etc/barbican/barbican.conf`, the dead `pyargv` is removed, worker and db-sync are unchanged, and no environment/argv secret or new volume is added.
- The MariaDB bootstrap gains idempotent barbican database/user creation plus the existing pod-template schema annotation bump (implemented as the `barbican-v1` schema value). The common bootstrap revision `v3` registers the barbican service user in the service project with admin, the `key-manager` service with internal/public/admin endpoints targeting the internal ClusterIP `http://<barbican-service>:9311`, and the compatibility roles `key-manager:service-admin`, `creator`, `observer`, and `audit`, with the Barbican password supplied only through the mounted credential file.
- Owned workloads are the pinned API Deployment/Service (uWSGI `9311` with an idempotent db-sync init container and venv-Python health probes) and worker Deployment only, hardened to UID/GID `42403` with supplemental group `42400`; the keystone-listener is omitted. No ServiceAccount, RBAC expansion, CRD field or toggle, or new readiness condition is added, and the stack is always-on for the bounded core profile. The dedicated Barbican Ingress emits `/barbican(/|$)(.*)` rewritten to `/$2` only after Service/API/worker readiness, and core readiness includes them. Implemented reconciliation: all six Barbican reads/preflight are fail-closed with explicit Service and Deployment duplicate-name guards, writes are dependency-safe, and the completion marker stays last.
