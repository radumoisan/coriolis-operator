...
- Accepted collision evidence: an unmanaged immutable v2 ConfigMap collision remained exact (UID, resourceVersion, creation time, key, and hash) and returned `ResourceCollision` with zero newly owned writes. Removing that conflict alone does not trigger reconciliation because metadata and child resources are not watched; controller resume or another watched reconciliation trigger is currently required. A normal operator Pod replacement invoked Kopf resume and converged. Track this collision-recovery behavior as an operational follow-up, not automatic recovery.
