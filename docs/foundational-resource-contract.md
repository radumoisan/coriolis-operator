...
## :material-book-open-page-variant-outline: Barbican Resource Contract (Source Implemented Locally, Release And Live Qualification Pending)

[ADR 0008](decisions/0008-operator-managed-barbican.md) freezes the Milestone 9 per-`CoriolisAppliance` Barbican naming, credential, ownership, and bootstrap boundary. The source implementation is complete locally and passes the local gates; nothing is released or deployed, and no live qualification is claimed.
...
- Owner-referenced rebuildable configuration: one ConfigMap with the non-sensitive API paste/vassal/policy/bootstrap assets and one config Secret with the rendered `barbican.conf`.
