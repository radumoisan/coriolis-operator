...
- **Milestone 11A extension (released `0.5.56` deployed, diagnostic-only pass on local corrections, corrective publication and runtime qualification pending; the bullets above remain unchanged history).** [ADR 0009](decisions/0009-log-sink-rotation-retention-ownership.md) makes Kubernetes stdout/stderr the application sink with node-local rotation owned by the kubelet/container runtime and query retention/compaction/deletion owned by Loki. `spec.logging` additionally accepts optional `compactionIntervalMinutes` (default 15, integer >= 1), `retentionDeleteDelayMinutes` (default 120, integer >= 1), and `coriolisDebug` (safe default false in the locally corrected source; the released `0.5.56` shipped default true, proven unsafe for value-safe acceptance against the immutable `2603.4` images); compaction may not exceed the retention period while delete delay may exceed retention, and invalid values fail closed into the mutation-free `InvalidRuntimeConfiguration` status. The Kubernetes `coriolis.conf` is stderr-only (`log_dir =` empty, `use_syslog = false`, `use_stderr = true`, `debug` from `coriolisDebug`), the in-container `/var/log/coriolis` log mounts are removed from core workloads, and the worker receives a dedicated `emptyDir` at `/var/log/coriolis/vmware-root` for VixDiskLib. All 14 natural producers require useful correlated records across pod logs, Loki, and the adaptor; memcached is the readiness/readability non-empty-log exception; the logging infrastructure is self-excluded but readiness/readability audited; synthetic markers stay bounded to qualification probes. The isolated `coriolis-m11a-20260908` deployment passed the deployed-shape checks, and with the observer-selector and default-false corrections — committed and pushed as `dd3530d659896b2ce806920ee6681a65256c82a0` but unpublished (no corresponding CIXpress pipeline discovered in two bounded GET-only observation windows; uncertain non-registration/absence, not a pipeline failure) — plus `debug=false` the natural-logs observer passed all 14 producers as diagnostics only; corrective publication, released-validator rerun, and formal retention qualification remain pending with no acceptance claim.
