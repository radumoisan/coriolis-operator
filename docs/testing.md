...
## :material-book-open-page-variant-outline: Local Coriolis Conductor Qualification And Reconciliation

- Exact pinned image `coriolis-conductor:2603.4@sha256:27495f44fbb8b320098d0aa04cd9dcb2a4b432e57aa17417606efc5403ac09c7` qualified three times through the extended tracked validator `scripts/validate-coriolis-bootstrap-runtime.py`. The final run completed in `272.660s` (`SUMMARY runtime passed`) and cleanup left no resources. Restricted direct `/usr/local/bin/coriolis-conductor --config-file=/etc/coriolis/coriolis.conf` ran as UID/GID `42434`, read-only root, dropped `ALL` capabilities, no-new-privileges, read-only generated configuration, and writable `/tmp`, `/var/log/coriolis`, and `/opt/coriolis/locks` on an internal network with no host port.
- A Keystone-authenticated `GET /v1/{project_id}/endpoints` returned HTTP `200`/list, proving the API→RabbitMQ→conductor→MariaDB path. It remained stable after a 20s settling period, a clean conductor SIGTERM/exit `0`/restart, and a RabbitMQ restart without conductor recreation. No secrets, tokens, or raw values were emitted.
- Local operator reconciliation applies one owner-referenced `Recreate` conductor Deployment only after a successful immutable common bootstrap and before the marker. It adds no Service/ports/probes/init/PVC/resources/CRD/RBAC/watch/finalizer, completes full preflight, and uses collision-safe guarded SSA.
- Explicit non-claims: this is local qualification and reconciliation only. There is no commit, release, CIXpress publication, deployed cluster artifact, POC, or `Ready=True`; write workflows/diagnostics and scheduler/workers/transfer-cron/minion/deployer remain absent. Overall status deliberately remains `Ready=False/RuntimeNotImplemented`. Final local validation passed: 516 unit tests, Ruff lint/format, strict mypy, Helm lint/template, operator container build, tracked runtime validator, and `git diff --check`.
...
- Normal cleanup removed CR-owned children in 5s and the exact Application, namespace, and both Delete-policy PVs without force/finalizer patches or individual retained/PV deletion; final Application/namespace/CR/PVs absent; shared `argocd/coriolis` resolves `0.5.22` `Synced/Healthy`, operator `1/1`, no CR, node Ready/schedulable. Authenticated workflow completeness beyond the verified read-only RPC smoke remains unclaimed; `Ready=False/RuntimeNotImplemented` remains required. The conductor slice is now locally qualified and implemented (see the section above) but is not published or released; remaining application workloads and Ingress follow.
