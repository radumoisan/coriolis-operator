...
- A deliberate unmanaged Keystone ConfigMap collision remained unchanged with `Reconciled=False/ResourceCollision` and zero managed writes; removing it restored healthy reconciliation. Cleanup removed the CR, disposable Application, namespace, retained Secrets/PVCs, and Delete-policy PVs; the node was Ready/schedulable, and shared Argo operator `0.5.14` was Synced/Healthy `1/1` with zero restarts, no appliance CR, and no POC workload. `Ready=False/RuntimeNotImplemented` remains truthful. CSI/cross-node, backup/restore, HA, RPO/RTO, credential/key rotation, and production storage remain open; the next implementation milestone is Coriolis-common bootstrap, then later Services/Ingress.

## :material-book-open-page-variant-outline: Coriolis-Common Bootstrap Validation (Local, Unreleased)

- The slice is implemented locally but uncommitted, unpushed, undeployed, and not POC-tested; it is absent from published docs-only operator `0.5.15` at `f7c369d`, whose runtime behavior remains accepted through the `0.5.14` Keystone POC. The Job selects the exact conductor `2603.4` digest `27495f44fbb8b320098d0aa04cd9dcb2a4b432e57aa17417606efc5403ac09c7`; `coriolis-common` remains a base image, not a workload. An owner ConfigMap and owner Job share `<appliance>-common-bootstrap-v1`, with the Job pinned to the conductor image, a create-only immutable template identity in the object and pod annotation, no patch/delete/TTL, file-only read-only Secret/config/script mounts, no credential env/argv/status/event/log/docs, no SA token/service links, a restricted security context, `300s` dependency wait, `120s` dbsync timeout, `600s` active deadline, and `backoffLimit: 2`.
- Reconciliation pre-reads the bootstrap ConfigMap and Job after Keystone and before writes, is collision-mutation-free, creates the immutable ConfigMap only when absent (exact managed reuse is no-write) and never patches it, and for the create-only Job requires an exact full managed-spec projection plus matching script/template IDs (any drift is mutation-free `ResourceCollision`); for an absent/active Job it returns `Progressing`/`BootstrapRunning` with the existing ten-second `TemporaryError` requeue and no marker; a terminal failed Job returns `Degraded`/`BootstrapFailed` with no marker; a succeeded Job is a no-writes path plus the marker last. RBAC adds only batch/jobs `get`/`create`; no CRD, application Service/Deployment, Ingress, new Secret, finalizer, or `Ready=True`.
- Tracked validator `scripts/validate-coriolis-bootstrap-runtime.py` ran the exact conductor as non-root UID/GID `42434`, read-only root, dropped capabilities/no-new-privileges, and only `/tmp` writable; the actual rendered script passed twice under all-read-only config/script/credential mounts across all four MariaDB/RabbitMQ/Memcached/Keystone protocol gates, the Coriolis schema dbsync, the exact Keystone user `coriolis` enabled with default project `service` and password convergence, the admin role, service name `coriolis` of type `migration` with description `Cloud Migration as a Service`, exactly one RegionOne endpoint per admin/internal/public at `http://coriolis-api:7667/v1/%(tenant_id)s`, and independent state verification. Final runtime `188.631s`; the actual bootstrap passed at `21.391s` and `25.273s`, verification at `10.416s`, with zero Docker leftovers. The Coriolis DB/user/grants remain existing MariaDB effects; the service-project creation is a necessary dedicated-Keystone precondition.
- Local validation passed: `uv run --offline pytest tests/unit` (477 passed), Ruff lint/format, mypy, Helm lint/template, Docker build, and `git diff --check`. This is local implementation and validator evidence only; no release, deployment, or cluster state changed, and no `Ready=True`. Publication and an isolated released-artifact POC remain pending.
