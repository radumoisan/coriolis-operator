...
- Accepted scope and limits: authenticated read-only conductor RPC is accepted; write workflows/diagnostics and scheduler/workers/transfer-cron/minion/deployer remain absent, and overall `Ready=False/RuntimeNotImplemented` remains required. The remaining application workloads then Ingress follow. Production CSI/cross-node, backup/restore, HA, RPO/RTO, and rotation remain deferred.
