...
The pure renderer is **committed locally at `9bb20f3` on `dev`, but not pushed or deployed**; deployed `0.5.3` remains marker-only. No `main.py`, reconciliation, Kubernetes reads/writes/SSA, RBAC, CRD, runtime resources/workloads, chart/release/image version, deployment, TLS/CA/bootstrap, provider/private data, optional credentials, storage, readiness, or rotation behavior changed. The multi-resource policy is frozen; collision-safe runtime reads/create/guarded SSA for all five resources plus the marker, minimal RBAC, status-then-Kopf-retry wiring, and exhaustive tests are next. Runtime integration remains blocked until they pass.

## :material-book-open-page-variant-outline: Foundational Multi-Resource Failure Contract (Documentation-Only)

- The policy in [Foundational Resource Contract](foundational-resource-contract.md) is frozen: validation; marker and five-resource pre-reads/classification; complete pure preflight, generation, rendering, and manifest preparation before mutation; canonical operations; marker last; no rollback or compensation; race-safe create/guarded-SSA retries; and value-safe `ResourceReadFailed`, `ResourceApplyFailed`, and `MarkerApplyFailed` status before framework retry. `ResourceCollision` remains non-transient and mutation-free; marker success is foundational completion only, and `Ready=False/RuntimeNotImplemented` remains truthful.
...
- Next implementation: collision-safe runtime reads and create/guarded SSA for all five resources plus the marker, minimal Secret/ConfigMap `get`/`create`/`patch` RBAC, status-then-Kopf-retry wiring, and exhaustive tests. Runtime integration remains blocked until it passes.
