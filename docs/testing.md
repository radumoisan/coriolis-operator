...
This pure slice is **committed locally at `35eac9b` on `dev`, but not pushed or deployed**. It changes no `main.py`, Kubernetes reads/writes, SSA, RBAC, CRD, runtime construction/adoption, reconciliation/status behavior, chart/release/image version, deployment, TLS/storage/readiness/bootstrap/rotation behavior; deployed `0.5.3` remains marker-only. The next gate is runtime integration of collision-safe Kubernetes pre-reads for all five before mutation, minimal Secret/ConfigMap `get` and SSA RBAC as required, SSA application, and reconciliation/status collision/failure semantics; retained adoption/runtime construction remains separately deferred.
