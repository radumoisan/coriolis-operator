...
- Normal cleanup removed CR-owned children in 5s and the exact Application, namespace, and both Delete-policy PVs without force/finalizer patches or individual retained/PV deletion; final Application/namespace/CR/PVs absent; shared `argocd/coriolis` resolves `0.5.22` `Synced/Healthy`, operator `1/1`, no CR, node Ready/schedulable. Authenticated API/RPC remains unclaimed until a conductor exists; `Ready=False/RuntimeNotImplemented` remains required. Next milestone is conductor qualification/contract/reconciliation, then remaining application workloads and Ingress.
